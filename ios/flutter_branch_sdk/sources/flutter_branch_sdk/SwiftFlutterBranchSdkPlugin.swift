...
public class SwiftFlutterBranchSdkPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
    ...
    public func application(_: UIApplication, didReceiveRemoteNotification userInfo: [AnyHashable: Any]) {
    ...
    private func showShareSheet(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        let response: NSMutableDictionary! = [:]
        buo?.showShareSheet(with: lp, andShareText: shareText, from: controller) { _, completed, error in
            if completed {
                response["success"] = NSNumber(value: true)
            } else {
                response["success"] = NSNumber(value: false)
    ...
    private func listOnSearch(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        if let lpDict = args["lp"] as? [String: Any?] {
            let lp: BranchLinkProperties! = convertToLp(dict: lpDict)
            buo!.listOnSpotlight(with: lp) { _, error in
                if error == nil {
            ...
            buo!.listOnSpotlight { _, error in
                if error == nil {
                    print("Successfully indexed on spotlight")
                    response = NSNumber(value: true)
                } else {
    ...
    private func setPreinstallCampaign(call _: FlutterMethodCall) {}

    private func setPreinstallPartner(call _: FlutterMethodCall) {}
